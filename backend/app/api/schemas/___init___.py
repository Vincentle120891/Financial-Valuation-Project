--- backend/app/api/schemas/__init__.py (原始)
...
+++ backend/app/api/schemas/__init__.py (修改后)
"""API Schemas module - Pydantic models for request/response validation."""
...
class ModelSelectRequest(BaseModel):
    """Request model for model selection."""
    session_id: str = Field(..., min_length=1, description="Session identifier")
    method: str = Field(..., min_length=1, description="Valuation method", examples=["DCF", "DuPont", "COMPS"])
    market: str = Field(default="international", description="Market version", examples=["international", "vietnam"])
    ...
    def validate_method(cls, v: str) -> str:
        ...
        allowed_methods = ["DCF", "DuPont", "COMPS", "dupont", "comps", "comparable", "trading comps", "dupont analysis"]
        if v.upper() not in [m.upper() for m in allowed_methods] and v.lower() not in allowed_methods:
            raise ValueError(f"Method must be one of: DCF, DuPont, COMPS")
        return v
