"""
Step 3: Vietnamese Historical Financial Data Processor - Refactored to Use Unified Schema
...
class vn_HistoricalYearData(BaseModel):
...
class vn_CalculatedMetrics(BaseModel):
...
class vn_Step3Response(BaseModel):
    """Vietnamese Step 3 response model."""
    ticker: str
    market_code: str
    historical_data: List[vn_HistoricalYearData]
    calculated_metrics: vn_CalculatedMetrics
...
class vn_Step3HistoricalProcessor:
    """
    ...
    def process_method_selection(
        ...
        """
        Process valuation method selection using unified schema (Vietnamese market).

        Args:
            session_id: Session identifier
            method: Valuation method (DCF, DUPONT, COMPS)
            market: Market type (vietnam, international)

        Returns:
            UnifiedStep3Response with method selection confirmation
        """
        logger.info(f"Processing Vietnamese method selection: session_id='{session_id}', method='{method}', market='{market}'")

        # Validate method
        available_methods = ["DCF", "DUPONT", "COMPS"]
        method_upper = method.upper()

        if method_upper not in available_methods:
            return UnifiedStep3Response(
                status="failed",
                ...
                message=f"Invalid method '{method}'. Available methods: {', '.join(available_methods)}"
            )

        # Method selection is always successful if valid
        return UnifiedStep3Response(
            status="success",
            session_id=session_id,
            method=method_upper,
            ...
            selected=True,
            available_methods=available_methods,
            message=f"Valuation method '{method_upper}' selected for {market.lower()} market (Vietnamese data)"
        )
    ...
    def process_historical_data(
        ...
        Returns:
            vn_Step3Response with historical data and metrics
        """
        ...
        historical_data_points: List[vn_HistoricalYearData] = []
        missing_years: List[str] = []
        missing_fields: Dict[str, List[str]] = {}
        warnings: List[str] = []
    ...
    def _extract_year_data(self, financials, year, ticker_data: Dict) -> vn_HistoricalYearData:
    ...
    def _calculate_vn_metrics(self, historical_data: List[vn_HistoricalYearData]) -> vn_CalculatedMetrics:
        """Calculate Vietnamese historical metrics."""
        if len(historical_data) < 2:
            return vn_CalculatedMetrics()
    ...
    def _check_tt99_compliance(self, historical_data: List[vn_HistoricalYearData]) -> Dict:
    ...
    def _convert_db_to_historical_data(self, db_data: Dict) -> List[vn_HistoricalYearData]:
        """Convert VNStockDatabase format to historical data points."""
        historical_data = []

        for year, data in db_data.items():
            historical_data.append(vn_HistoricalYearData(
                year=str(year),
