...
class VNValuationModel(str, Enum):
...
class VNForecastDriver(BaseModel):
...
class VNDuPontInputs(BaseModel):
...
class VNStep4Response(BaseModel):
    """Vietnamese Step 4 response model"""
    ticker: str
    market_code: str  # VN, HA, VC
    valuation_model: VNValuationModel
    forecast_drivers: Optional[List[VNForecastDriver]] = None
    revenue_growth_forecast: Optional[List[Dict]] = None
    margin_assumptions: Optional[Dict] = None
    comps_inputs: Optional[VNCompsInputs] = None
    dupont_inputs: Optional[VNDuPontInputs] = None
    warnings: List[str] = []
...
class VNStep4ForecastProcessor:
    """
    ...
    def process_forecast_drivers(
        ...
        Returns:
            VNStep4Response with model-specific inputs
    ...
    def _process_vn_dcf_inputs(
        ...
        Returns:
            VNStep4Response with DCF forecast drivers
        """
        ...
        return VNStep4Response(
            ticker=ticker.upper(),
            market_code=market_code,
            valuation_model=VNValuationModel.DCF,
    ...
    def _process_vn_comps_inputs(
        ...
        Returns:
            VNStep4Response with Comps configuration
        """
        warnings = []
        data_quality = 85.0
        ...
        return VNStep4Response(
            ticker=ticker.upper(),
            market_code=market_code,
            valuation_model=VNValuationModel.COMPS,
    ...
    def _process_vn_dupont_inputs(
        ...
        Returns:
            VNStep4Response with DuPont configuration
        """
        warnings = []
        data_quality = 85.0
        ...
        vn_dupont_inputs = VNDuPontInputs(
            years_to_analyze=3,
        ...
        return VNStep4Response(
            ticker=ticker.upper(),
            market_code=market_code,
            valuation_model=VNValuationModel.DUPONT,
