"""
Vietnamese Step 3: Model Selection Processor

Handles model selection for Vietnamese companies (Step 3, BEFORE peer discovery in Step 4).
This aligns with the International workflow: Model Selection → Peer Discovery.
...
Bridge method: process_model_selection() matches the INT Step3SelectedModelsProcessor
signature so valuation_routes.py can delegate when market='vietnam'.
"""
...
class vn_Step3ModelSelectionProcessor:
    """
    Processor for Vietnamese Step 3: Model Selection
    ...
    def __init__(self, session_service=None):
        ...
        logger.info("vn_Step3ModelSelectionProcessor initialized for Vietnamese market")
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # Bridge method — matches INT Step3SelectedModelsProcessor signature
    # ──────────────────────────────────────────────────────────────────────────
    ...
    def process_model_selection(
        ...
        """
        Bridge method matching INT Step3SelectedModelsProcessor.process_model_selection().

        Validates the selected models and returns a result dict with the same
        shape the shared route handler expects.

        Args:
            selected_models: List of model names (e.g. ["DCF"], ["DUPONT", "COMPS"])

        Returns:
            Dict with keys: is_valid, validated_models, invalid_models, model_configs
        """
        model_list = [m.lower() for m in selected_models]
        invalid = [m for m in model_list if m not in self.AVAILABLE_MODELS]

        return {
            "is_valid": len(invalid) == 0,
            "validated_models": [m for m in model_list if m in self.AVAILABLE_MODELS],
            "invalid_models": invalid,
            "model_configs": self._build_vn_configurations(
                [m for m in model_list if m in self.AVAILABLE_MODELS]
            ),
        }