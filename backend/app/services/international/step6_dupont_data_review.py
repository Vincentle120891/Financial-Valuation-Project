...
class DuPontStep6Processor:
    ...
    async def process_dupont_data_review(
        ...
        if historical_data is None or market_data is None:
            ...
            DUPont_INCOME = {
                ...
                "operating_income": "ebit", "ebitda": "ebitda",
            ...
            financials_df = build_multiyear_df(yfs_income, DUPont_INCOME)
            balance_sheet_df = build_multiyear_df(yfs_bs, DUPont_BS)
            cashflow_df = build_multiyear_df(yfs_cf, {"operating_cash_flow": "operating_cash_flow"})
    ...
    def _extract_dupont_metric(
        ...
        mapping = {
            "revenue": (financials_df, ["revenue", "Total Revenue", "Revenue"]),
            "net_income": (financials_df, ["net_income", "Net Income", "Net Income Common Stockholders"]),
            "total_assets": (balance_sheet_df, ["total_assets", "Total Assets", "Assets"]),
            "shareholders_equity": (balance_sheet_df, ["shareholders_equity", "Stockholders Equity", "Total Equity"]),
            "total_liabilities": (balance_sheet_df, ["total_liabilities", "Total Liabilities", "Liabilities"]),
            "retained_earnings": (balance_sheet_df, ["retained_earnings", "Retained Earnings"])
        }
