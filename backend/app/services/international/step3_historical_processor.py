...
class Step3Response(BaseModel):
    """Step 3 response model (LEGACY - kept for backward compatibility)."""
    ticker: str
...
class Step3HistoricalProcessor:
    ...
    def process_method_selection(
        ...
        """
        Process valuation method selection using unified schema.
        ...
        """
        logger.info(f"Processing method selection: session_id='{session_id}', method='{method}', market='{market}'")
        ...
        return UnifiedStep3Response(
            ...
            selected=True,
            available_methods=available_methods,
            message=f"Valuation method '{method_upper}' selected for {market.lower()} market"
        )
