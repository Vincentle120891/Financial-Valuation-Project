...
import type { DataField } from '@/types/backend-contracts';
 ...
 */
const DataFieldDisplay: React.FC<DataFieldDisplayProps> = ({ 
  dataField, 
  label, 
  unit, 
  showMetadata = true, 
  ...
  const hasValues = () => {
    if (Array.isArray(dataField.value)) {
      return dataField.value.some((pv: any) => pv.value !== null && pv.value !== undefined);
    }
  ...
  const formatValue = (value: any, fieldUnit?: string) => {
  ...
  const getPeriods = () => {
    ...
    if (Array.isArray(dataField.value)) {
      return dataField.value
        .filter((pv: any) => pv.period !== null && pv.period !== undefined)
        .map((pv: any) => pv.period);
    }
    return ['Current'];
  };
  ...
  // Size configurations
  const sizeConfig: Record<string, { padding: string; fontSize: string; periodFontSize: string; valueFontSize: string }> = {
        ...
        <>
          <div style={{ 
            ...
            {(Array.isArray(dataField.value) ? dataField.value : [{ period: 'Current', value: dataField.value }])
              .filter((pv: any) => pv.value !== null && pv.value !== undefined)
              .map((periodValue: any, idx: number) => {
 ...
 */
export const DataFieldGrid: React.FC<DataFieldGridProps> = ({ fields, title, size = 'medium' }) => {
  return (
