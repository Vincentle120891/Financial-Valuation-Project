...
const ResultsStep: React.FC<ResultsStepProps> = ({
  ...
  // Normalize selectedModels to always be an array (store may hold a string like 'DCF')
  const models: string[] = React.useMemo(() => {
    if (Array.isArray(selectedModels)) return selectedModels;
    if (typeof selectedModels === 'string' && selectedModels.trim()) {
      return selectedModels.split(',').map(m => m.trim()).filter(Boolean);
    }
    return [];
  }, [selectedModels]);
  ...
  const hasAnyResults = models.some(model => getResultsForMethod(model.toLowerCase()));

  if (!hasAnyResults) {
    return (
      <div className="step-container">
        <h2>{t('steps.step11')}</h2>
        <p style={{ color: 'var(--text-secondary)', textAlign: 'center', padding: '40px' }}>
          {t('messages.no_data_retrieved')}. {t('messages.please_go_back')}
        </p>
  ...
  const prepareDupontTrendData = (dupontOutputs) => {
    ...
    return Object.entries(roeHistory).map(([year, roe]: [string, any]) => ({
      year,
      roe: (Number(roe) * 100).toFixed(1),
  ...
  return (
    <div className="step-container">
      ...
      <p style={{ marginBottom: '24px', color: 'var(--text-secondary)' }}>
        {t('stepDescriptions.step11')}
      </p>
      
      {/* Multi-Method Summary Dashboard */}
      {models.length > 1 && (
        <div className="summary-box" style={{ marginBottom: '32px', background: 'var(--accent-primary)', color: 'white' }}>
          <h3 style={{ color: 'white', marginBottom: '20px' }}>📊 {t('sections.multi_method_summary')}</h3>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', color: 'white' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid rgba(255,255,255,0.3)' }}>
                  <th style={{ padding: '12px', textAlign: 'left' }}>{t('tableHeaders.sector')}</th>
                  {models.map(model => (
      ...
      {models.map((model) => {
        ...
        return (
          <div key={model} className="results-dashboard" style={{ marginBottom: '32px', paddingBottom: '32px', borderBottom: models.indexOf(model) < models.length - 1 ? '2px solid var(--border-default)' : 'none' }}>
                  ...
                  return (
                    <div className="summary-box" style={{ marginTop: '24px' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginTop: '20px' }}>
                        <div style={{ padding: '12px', background: 'var(--canvas-surface)', borderRadius: '8px' }}>
                          <strong style={{ color: 'var(--text-secondary)', fontSize: '12px' }}>Avg ROE</strong>
                          <p style={{ fontSize: '20px', fontWeight: 'bold', color: 'var(--accent-primary)' }}>
                            {(dupontTrendData.reduce((sum, d) => sum + parseFloat(d.roe), 0) / dupontTrendData.length).toFixed(1)}%
                          </p>
                        </div>
                        <div style={{ padding: '12px', background: 'var(--canvas-surface)', borderRadius: '8px' }}>
                          <strong style={{ color: 'var(--text-secondary)', fontSize: '12px' }}>ROE Trend</strong>
                          <p style={{ fontSize: '20px', fontWeight: 'bold', color: parseFloat(dupontTrendData[dupontTrendData.length - 1]?.roe) > parseFloat(dupontTrendData[0]?.roe) ? 'var(--color-bullish)' : 'var(--color-bearish)' }}>
                            {parseFloat(dupontTrendData[dupontTrendData.length - 1]?.roe) > parseFloat(dupontTrendData[0]?.roe) ? '📈 Improving' : '📉 Declining'}
                          </p>
                        </div>
                        <div style={{ padding: '12px', background: 'var(--canvas-surface)', borderRadius: '8px' }}>
                          <strong style={{ color: 'var(--text-secondary)', fontSize: '12px' }}>Latest ROE</strong>
                          <p style={{ fontSize: '20px', fontWeight: 'bold', color: 'var(--color-manual)' }}>{dupontTrendData[dupontTrendData.length - 1]?.roe}%</p>
                        </div>
                      </div>
                    </div>
                  );
