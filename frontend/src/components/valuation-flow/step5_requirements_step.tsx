...
const RequirementsStep: React.FC<RequirementsStepProps> = ({
  ...
  const hasRetrievedData = dataFetched || !!calculatedMetrics;
  ...
  const getCategoryConfig = (categoryName: string) => {
    const configMap: Record<string, { color: string; icon: React.ReactNode; label: string; gradient: string }> = {
  ...
  const getStatusBadge = (fieldId: string) => {
  ...
  const getApiSourceBadge = (fieldName?: string) => {
  ...
  const groupedFields = requiredFields.reduce<Record<string, RequiredField[]>>((groups, field) => {
    ...
    >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('steps.step5')}</h2>
      ...
      <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg">
        <div className="flex items-start">
          ...
          <div className="ml-3">
            <h3 className="text-sm font-medium text-blue-800">{t('messages.info')}</h3>
      ...
      <div className="flex items-center justify-between pt-4">
        ...
        <div className="flex items-center space-x-4">
          <div className="text-sm text-gray-500 hidden sm:block">
            {hasRetrievedData
              ? <span className="text-green-600 font-semibold">Data fetched ✓</span>
          ...
          {!hasRetrievedData ? (
            ...
            <button
              onClick={onContinueToStep6}
              disabled={loading}
              className="btn-next-step"
            >
              Continue to API Data Review →
            </button>
          )}
        </div>
      </div>
