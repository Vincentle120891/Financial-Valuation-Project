...
interface PeerSelectionStepProps {
  ...
  onBack: () => void;
  loading?: boolean;
  onFindPeers?: ((company: any) => Promise<void>) | null;
  selectedCompany?: any;
  market?: string;
}
 ...
 */
const PeerSelectionStep: React.FC<PeerSelectionStepProps> = memo(({
  discoveredPeers = [],
  selectedPeers = [],
  onTogglePeer,
  onSelectAll,
  ...
  const isInvalidPeer = (ticker: string) => {
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-slate-900 mb-2">{t('steps.step4')}</h2>
          <p className="text-slate-600">{t('stepDescriptions.step4')}</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-slate-900 mb-2">{t('messages.no_data_retrieved')}</h3>
  ...
  return (
    ...
    <ProcessingOverlay
      ...
      subtitle="Searching for comparable companies..."
      messages={[
        'Analyzing industry classifications and market segments...',
        'Comparing financial metrics and market capitalization...',
        'Evaluating company size, sector, and geography...',
        'Computing similarity scores across peer candidates...',
        'Filtering and ranking comparable companies...',
      ]}
      accentColor="purple"
    />
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm font-medium text-slate-900">
                        {(() => {
                          const rawCap = peer.market_cap || peer.marketCap;
                          // Unwrap DataField object: {value: 123, status: "RETRIEVED", ...} → 123
                          const cap = (rawCap && typeof rawCap === 'object' && 'value' in rawCap) ? rawCap.value : rawCap;
                          if (!cap || typeof cap !== 'number') return 'N/A';
                        ...
                        <div className="flex-1 h-2 bg-slate-200 rounded-full overflow-hidden max-w-[100px]">
                          <div
                            className={`h-full rounded-full transition-all ${
                              (peer.match_score || 0) >= 80 ? 'bg-green-500' :
                              (peer.match_score || 0) >= 60 ? 'bg-yellow-500' :
                              'bg-orange-500'
                            }`}
                            style={{ width: `${Math.min(peer.match_score || peer.similarity_score || 0, 100)}%` }}
                          />
                        </div>
                        <span className="text-sm font-medium text-slate-700 min-w-[3rem]">
                          {(peer.match_score || peer.similarity_score || 0).toFixed(0)}%
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1 max-w-xs">
                        {(peer.match_reasons || []).slice(0, 2).map((reason: string, idx: number) => (
                          <span
                            key={idx}
      ...
      <div className="flex justify-between items-center pt-4">
        ...
        <div className="flex gap-4">
          ...
          >
            {selectedPeers.length === 0 ? 'Select at Least One Peer →' : 'Continue to Requirements →'}
          </button>
        </div>
      </div>
    </div>
...
PeerSelectionStep.displayName = 'PeerSelectionStep';
