...
const ApiKeyModal = ({ isOpen, onClose, onSave }) => {
  ...
  const getServiceDescription = (service) => {
    const descriptions = {
      fmp: 'Financial Modeling Prep - For peer discovery, financial statements, and market data',
      fred: 'Federal Reserve Economic Data - For US Treasury yields (Risk-Free Rate)',
      alphaVantage: 'Alpha Vantage - For additional market data and technical indicators',
      secEdgar: 'SEC EDGAR Email - For accessing SEC filings and company disclosures (format: your-email@domain.com)'
    };
  ...
  const getServiceLink = (service) => {
    const links = {
      ...
      secEdgar: 'https://www.sec.gov/edgar/sec-api-documentation'
    };
            ...
            <div key={service} className="space-y-2">
              <div className="flex justify-between items-center">
                <label className="block text-sm font-medium text-gray-700 capitalize">
                  {service === 'fmp' ? 'Financial Modeling Prep (FMP)' : 
                   service === 'fred' ? 'FRED API' :
                   service === 'alphaVantage' ? 'Alpha Vantage' : 'SEC EDGAR'}
                </label>
                {savedStatus[service] && (
