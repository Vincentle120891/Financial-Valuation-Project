...
import {
  ...
  generateScenarios as generateScenariosApi
} from './api';

/**
 * Transforms Vietnamese market valuation response to Unified Schema format.
 * Vietnamese API returns flat structure; we need to wrap it in the expected format.
 * NOTE: This is distinct from transformVietnameseResponse in api.js which normalizes search results.
 */
const transformVietnameseValuationResponse = (response, method) => {
...
export const retrieveData = async ({
  ...
  try {
    ...
    if (market && market.toLowerCase() === 'vietnam') {
      transformedData = transformVietnameseValuationResponse(fetchDataResponse, method);
    } else {
...
export const generateAISuggestion = async (sessionId, category, method, market = 'international') => {
  try {
    ...
    // Return raw API response directly (has status, suggestion, ai_details, message)
    return response;
  } catch (error) {
 ...
 * Wrapper around generateScenarios API call
 */
export const generateScenarios = async (sessionId, method, market = 'international', baseCase = {}) => {
  try {
    const response = await generateScenariosApi(sessionId, method, market, baseCase);
    return {
      success: true,
      data: response
    };
