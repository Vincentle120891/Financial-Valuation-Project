...
- **Rendering Logic**:  
- // Before (Fragile)  
 <div>{data.revenue_2023 || data.revenue?.[2]?.value}</div>  
   
 // After (Robust)  
...
| | | | |  
|-|-|-|-|  
| **Phase** | **Step** | **Action Item** | **Estimated Effort** |   
| **1** | 1.1 | Create unified_step_schemas.py with all 10 steps | High |   
| **1** | 1.2 | Refactor International Step 6 (DCF/DuPont/Comps) | High |   
| **1** | 1.3 | Refactor Vietnamese Step 6 (All models) | High |   
| **1** | 1.4 | Verify API responses match schema (Postman tests) | Medium |   
| **2** | 2.1 | Update Frontend TypeScript definitions | Low |   
| **2** | 2.2 | Refactor API Service layer to remove branching | Medium |   
| **2** | 2.3 | Update Step 6 UI Components to use nested map | High |   
| **2** | 2.4 | Repeat Steps 1.2-2.3 for remaining Steps (1-5, 7-10) | Very High |   
| **3** | 3.1 | End-to-End Testing (Intl & VN, all 3 methods) | High |   
| **3** | 3.2 | Performance profiling (ensure nesting doesn't bloat) | Low |   
   
